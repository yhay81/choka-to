...
if (-not $Worker.Contains('class="water-scene"') -or
    -not $Worker.Contains('class="catch-board"') -or
    -not $Worker.Contains('class="catch-stream"') -or
    -not $Worker.Contains('class="pattern-dock"') -or
    -not $Worker.Contains('class="local-flow"')) {
    throw "Expected the visual water, catch board, catch stream, pattern dock, and data flow"
}
...
if ($App -match '(?i)innerHTML|eval\(|new Function') {
    throw "Fishing content must not be interpreted as markup or code"
}
if (([regex]::Matches($App, '(?i)\bfetch\s*\(').Count -ne 1) -or
    -not $App.Contains('fetch("/api/events"')) {
    throw "The log may send only anonymous allowlisted product events"
}
if (-not $App.Contains("indexedDB.open") -or
    -not $App.Contains('createObjectStore("trips"') -or
    -not $App.Contains('createObjectStore("catches"') -or
    -not $App.Contains('createObjectStore("photos"') -or
    -not $App.Contains("const maximumTrips = 100") -or
    -not $App.Contains("const maximumCatchesPerTrip = 100") -or
    -not $App.Contains("const maximumCatches = 500") -or
    -not $App.Contains("const maximumPhotos = 100")) {
    throw "Expected bounded local trip, catch, and photo storage"
}
if (-not $App.Contains("createImageBitmap") -or
    -not $App.Contains("canvas.toBlob") -or
    -not $App.Contains("maximumPhotoBytes = 220 * 1024") -or
    -not $App.Contains(".chokato") -or
    -not $App.Contains("text/csv") -or
    -not $App.Contains("window.print()") -or
    -not $App.Contains("位置を含めない共有札")) {
    throw "Expected local photo reduction and complete share/export/print paths"
}
if (-not $ServiceWorker.Contains('const cacheName = "choka-to-v1"') -or
...
if ($Migration -match '(?i)\b(trip|catch|species|spot|water|tackle|lure|weight|photo|filename|latitude|longitude|location|email|phone|user_agent)\b') {
    throw "Fishing content, identity, location, and file metadata do not belong in telemetry"
}
...
if (-not $Worker.Contains('geolocation=()') -or
    $ProductSurface -match 'navigator\.geolocation|getCurrentPosition|watchPosition') {
    throw "The release must not request precise location"
}
if (-not $Styles.Contains("@page") -or
    -not $Styles.Contains("size: A4 portrait") -or
    -not $Styles.Contains("@media print")) {
    throw "Expected an explicit printable trip report"
}
