...
const eventRequest = (
  ...
  headers: {
    ...
    "x-choka-qa": options.qa ? "1" : "0",
    "x-choka-session": options.session ?? session,
  },
  method: "POST",
});
...
describe("public pages", () => {
  it.each([
    ["/", 'class="water-scene"', "https://choka-to.yhay81.com/"],
    ["/guide", 'class="guide-steps"', "https://choka-to.yhay81.com/guide"],
    ["/privacy", 'class="privacy-flow"', "https://choka-to.yhay81.com/privacy"],
  ...
  it("記録画面は水面、釣果盤、魚種の並び、持出し導線を持つ", async () => {
    const response = await app.request("/", undefined, bindings);
    const html = await response.text();
    expect(response.headers.get("cache-control")).toBe("no-store");
    expect(html).toContain('class="catch-board"');
    expect(html).toContain('class="catch-stream"');
    expect(html).toContain('class="pattern-dock"');
    expect(html).toContain('class="local-flow"');
    expect(html).toMatch(/<script src="\/app\.js" type="module"><\/script>/);
    expect(html).toContain("写真込み .chokato");
    expect(html).toContain("表計算用 CSV");
    expect(html).toContain("位置なし共有札");
    expect(html).toContain("釣行票を印刷 / PDF");
  });

  it("未知のページは404、静的アセットはASSETSへ渡す", async () => {
    const page = await app.request("/missing", undefined, bindings);
    expect(page.status).toBe(404);
    expect(await page.text()).toContain("この水面には、まだ記録がありません");
...
describe("anonymous telemetry", () => {
  ...
  it("イベント名、本文field、セッションIDを許可リストで制限する", async () => {
    ...
    const content = await app.request(
      "/api/events",
      eventRequest("catch_added", {
        body: JSON.stringify({ name: "catch_added", species: "シーバス" }),
      }),
      bindings,
    );
  ...
  it("JSON以外、不正JSON、1KB超の本文を拒否する", async () => {
    const media = await app.request(
      ...
      {
        body: "name=visited",
        headers: { "content-type": "text/plain", "x-choka-session": session },
        method: "POST",
      },
      bindings,
    );
  ...
  it("自動QAイベントを実利用から分離する", async () => {
    await app.request("/api/events", eventRequest("catch_added", { qa: true }), bindings);
    await app.request("/api/events", eventRequest("catch_added"), bindings);
...
describe("local catch log contract", () => {
  it("釣行内容を送らず、IndexedDB内の釣行・釣果・写真だけを扱う", async () => {
    ...
    expect(source).toContain('createObjectStore("trips"');
    expect(source).toContain('createObjectStore("catches"');
    expect(source).toContain('createObjectStore("photos"');
    expect(source).toContain("const maximumTrips = 100");
    expect(source).toContain("const maximumCatchesPerTrip = 100");
    expect(source).toContain("const maximumCatches = 500");
    expect(source).toContain("const maximumPhotos = 100");
    expect(source).not.toMatch(/innerHTML|eval\(|new Function/);
    expect(source).not.toMatch(/navigator\.geolocation|getCurrentPosition|watchPosition/);
  });

  it("写真を端末内で再圧縮し、共有札・全バックアップ・CSV・PDFを持ち出せる", async () => {
    ...
    expect(source).toContain("maximumPhotoBytes = 220 * 1024");
    expect(source).toContain(".chokato");
    expect(source).toContain("text/csv");
    expect(source).toContain("window.print()");
    expect(source).toContain("/^[=+\\-@]/");
    expect(source).toContain("場所メモ・正確な時刻・写真は含めていません");
    expect(styles).toContain("size: A4 portrait");
    expect(styles).toContain("@media print");
  });

  it("読み込み前に形式、上限、UUID、参照、写真対応を検証する", async () => {
    const source = await readFile(appPath, "utf8");
    expect(source).toContain('project.format !== "choka-to"');
    expect(source).toContain("project.trips.length > maximumTrips");
    expect(source).toContain("project.catches.length > maximumCatches");
    expect(source).toContain("project.photos.length > maximumPhotos");
    expect(source).toContain("validImportedTrip");
    expect(source).toContain("validImportedCatch");
    expect(source).toContain("tripIds.size !== project.trips.length");
    expect(source).toContain("photoIds.has(record.id)");
  });

  it("静的製品面をネットワーク優先でオフラインキャッシュする", async () => {
    const source = await readFile(serviceWorkerPath, "utf8");
    expect(source).toContain('const cacheName = "choka-to-v1"');
...
describe("discovery assets", () => {
  it("manifestは釣果灯のPWA情報を持つ", async () => {
    const manifest = JSON.parse(await readFile(manifestPath, "utf8"));
    expect(manifest.name).toBe("釣果灯");
    expect(manifest.start_url).toBe("/");
    expect(manifest.display).toBe("standalone");
    expect(manifest.icons).toHaveLength(1);
  });

  it("sitemapとrobotsはchoka-to.yhay81.comを指す", async () => {
    const [sitemap, robots] = await Promise.all([
      readFile(sitemapPath, "utf8"),
      readFile(robotsPath, "utf8"),
    ]);
    expect(sitemap.match(/<loc>/g)).toHaveLength(3);
    expect(sitemap).toContain("https://choka-to.yhay81.com/guide");
    expect(robots).toContain("https://choka-to.yhay81.com/sitemap.xml");
  });

  it("OG画像は水面、魚影、釣果札で製品を示す", async () => {
    const source = await readFile(ogPath, "utf8");
    expect(source.length).toBeGreaterThan(2500);
    expect(source).toContain("PRIVATE CATCH LOG");
    expect(source).toContain("シーバス");
    expect(source).toContain("GPS要求なし");
  });
});
